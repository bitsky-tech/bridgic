...
The event handling mechanism enables real-time communication between workers inside the Automa and the application layer outside the Automa. Events can be sent from a worker to the application layer, and feedback can be sent in the opposite direction, from the application layer back to the worker, if needed.

A worker can use the `post_event()` method to send an event to the application layer. To handle these events, the application layer can register event handlers by calling the Automa's `register_event_handler()` method.
...
The Bridgic event handling mechanism requires the Agent process to remain alive in memory for the entire duration of the event and feedback interaction. If the interaction period is too long and the Agent process needs to be shut down in the middle, the Bridgic human interaction mechanism should be used instead.
