...
from bridgic.automa import GraphAutoma, worker, WorkerArgsMappingError, ArgsMappingRule
...
class Flow_1_Test_AS_IS(GraphAutoma):
    ...
    def func_1(self, x: int, y):
        # Return a dict.
        return {"x": x, "y": y}

    @worker(dependencies=["func_1"])
    def func_2(self, map: dict):
        assert map == {"x": 1, "y": 3}
        # Return a list.
        return [map["x"], map["y"]]

    @worker(dependencies=["func_2"])
    def func_3(self, my_list):
        assert my_list == [1, 3]
        # Return a list.
        return my_list

    @worker(dependencies=["func_3"])
    def func_4(self, *args):
    ...
    def func_5(self, my_list: List[int], *args):
    ...
    def func_6(self, coord: Coordinate):
        assert coord.x == 1
        assert coord.y == 3
        return coord

    @worker(dependencies=["func_6"])
    def func_7(self, coord: Coordinate=Coordinate(11, 33)):
    ...
    def func_8(self, coord, coord2: Coordinate=Coordinate(11, 33)):
        # Test case that a default value is provided for the parameter.
        assert coord.x == 1
        assert coord.y == 3
        assert coord2.x == 11
    ...
    def func_9(self, value):
        assert value is None
        # return None again

    @worker(dependencies=["func_9"])
    def func_10(self, *args):
...
async def test_flow_1(flow_1):
    result = await flow_1.process_async(x=1, y=3)
...
class Flow_2_Test_AS_IS(GraphAutoma):
    ...
    """
    @worker(is_start=True)
    def start(self, x: int, y):
        return Coordinate(x, y)

    @worker(dependencies=["start"])
    def func_1(self, coord: Coordinate):
        assert coord.x == 2
        assert coord.y == 3
        return coord.x + coord.y

    @worker(dependencies=["start"])
    def func_2(self, coord: Coordinate):
        assert coord.x == 2
        assert coord.y == 3
        # return None test

    @worker(dependencies=["start"])
    def func_3(self, coord: Coordinate):
        assert coord.x == 2
        assert coord.y == 3
        return coord.x * coord.y
...
async def test_flow_2(flow_2):
    result = await flow_2.process_async(x=2, y=3)
...
async def test_flow_3(flow_3):
    coordinates = await flow_3.process_async(x=2, y=3)
...
async def test_flow_4(flow_4):
    with pytest.raises(WorkerArgsMappingError, match="expects at least 2 arguments"):
        await flow_4.process_async(x=2, y=3)
...
async def test_flow_5(flow_5):
    with pytest.raises(WorkerArgsMappingError, match="expects at least 0 arguments"):
        await flow_5.process_async(x=2, y=3)
...
class Flow_A_Test_UNPACK(GraphAutoma):
    ...
    def func_2(self, x: int, y: int, z: int=0):
        # Return a dict again.
...
async def test_flow_A(flow_A):
    coordinates = await flow_A.process_async(x=2, y=3)
...
async def test_flow_B(flow_B):
    with pytest.raises(WorkerArgsMappingError, match="must has exatly one dependency"):
        await flow_B.process_async(x=2, y=3)
...
async def test_flow_C(flow_C):
    with pytest.raises(WorkerArgsMappingError, match="only valid for tuple/list, or dict"):
        await flow_C.process_async(x=2, y=3)
...
class Flow_I_Test_MERGE(GraphAutoma):
    ...
    def end1(self, coordinates: List[Coordinate]):
...
async def test_flow_I(flow_I):
    coordinates = await flow_I.process_async(x=2, y=3)
    assert len(coordinates) == 3
...
class Flow_II_Test_MERGE(GraphAutoma):
    ...
    def merge(self, coordinates: List[Coordinate]):
        return coordinates
...
async def test_flow_II(flow_II):
    coordinates = await flow_II.process_async(x=2, y=3)
    coord1, coord2, coord3 = coordinates
    assert coord1.x == 2
    assert coord1.y == 3
    assert coord2.x == 3
...
async def test_flow_III(flow_III):
    with pytest.raises(WorkerArgsMappingError, match="must has at least 2 dependencies"):
        await flow_III.process_async(x=2, y=3)

class Flow_IV_ErrorTest_MERGE(GraphAutoma):
    @worker(is_start=True)
    def start1(self, x: int, y):
        return [x, y]

    @worker(is_start=True)
    def start2(self, x: int, y):
        return [x, y]
...
async def test_flow_IV(flow_IV):
    with pytest.raises(WorkerArgsMappingError, match="at least one positional parameter"):
        await flow_IV.process_async(x=2, y=3)
...
async def test_flow_101(flow_101):
    result = await flow_101.process_async(x=2, y=3)
    assert result == (2, 3)
