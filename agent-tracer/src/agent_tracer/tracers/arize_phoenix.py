...
class ArizePhoenixTracer(BaseTracer):
    """Arize Phoenix tracer for observability."""
    ...
    def __init__(
        ...
        self.flow_name = str(trace_name).split(" - ")[0]
        ...
        try:
            ...
            # Import SpanAttributes after phoenix is set up
            ...
            self.root_span.set_attribute("agent_tracer.project.name", self.project_name)
            self.root_span.set_attribute("agent_tracer.flow.name", self.flow_name)
            self.root_span.set_attribute("agent_tracer.flow.id", self.flow_id)
    ...
    def setup_arize_phoenix(self) -> bool:
        ...
        try:
            ...
            logger.exception(
                "Could not import arize-phoenix-otel. Please install it with `pip install agent-tracer[arize-phoenix]`."
            )
            return False
        ...
        try:
            ...
            logger.debug(
                "LangChainInstrumentor not available. "
                "Install with `pip install openinference-instrumentation-langchain` for LangChain integration."
            )
    ...
    def add_trace(
        ...
        if component_name == "ChatInput":
            self.chat_input_value = processed_inputs.get("input_value", "")
        elif component_name == "ChatOutput":
            self.chat_output_value = processed_inputs.get("input_value", "")
