...
class LangSmithTracer(BaseTracer):
    ...
    def __init__(
        ...
        try:
            ...
            if parent is not None and (parent.id == trace_id or parent.name == trace_name):
                # duplicate init of LangSmithTracer with same trace_id/trace_name, using current run tree
    ...
    def setup_langsmith(self) -> bool:
        ...
        try:
            ...
            logger.exception("Could not import langsmith. Please install it with `pip install agent-tracer[langsmith]`.")
            return False
    ...
    def end_trace(
        ...
        if logs:
            ...
            child.add_metadata(
                self._convert_to_langchain_types({"logs": {log.get("name"): log for log in logs_dicts}})
            )
    ...
    def end(
        ...
        if not self._ready or not self._run_tree:
            return
        self._run_tree.add_metadata({"inputs": serialize_value(inputs)})
        if metadata:
            self._run_tree.add_metadata(serialize_value(metadata))
        self._run_tree.end(outputs=serialize_value(outputs), error=self._error_to_string(error))
